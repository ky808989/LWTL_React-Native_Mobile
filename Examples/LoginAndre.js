...
const DEFAULT_USER = 'andreliem';
const DEFAULT_PW = 'andreliem';




class LoginAndre extends Component {
		constructor(props){
			super(props);
			this.state = {
		...
		getToken(){
			...
			if (responseJson['error'] != null || responseJson['token'] === undefined){
				AlertIOS.alert(responseJson['error']);
					...
					try {
						AsyncStorage.setItem(STORAGE_TOKEN, responseJson['token']);
				} catch (error){
